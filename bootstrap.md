...
- Seed the two human-owned policy files from their templates and tell the user both are theirs to edit: `guardrails.md` (rules the work must obey) and `pr-review-strategy.md` (the {{PR_NOUN}} reviewer lineup per area). Neither is ever regenerated by a refresh.
...
- [ ] `skills/personas/` has 5 generic personas; `skills/templates/` has 10 templates (including `project-guardrails-template.md` and `project-pr-review-strategy-template.md`).
