...
├── pr-review-strategy.md # prescriptive, human-owned {{PR_NOUN}} reviewer lineup (seeded here, then user-edited)
...
### 7b. Seed the {{PR_NOUN}} review strategy

Write `projects/<name>/pr-review-strategy.md` from `skills/templates/project-pr-review-strategy-template.md`: an **area map** built from the repo's real top-level layout, a **reviewer lineup** binding the personas from step 7 to those areas (reuse them in review mode; add a review-only row only for a lens no persona covers) (always-on lead + guardrails + architecture + test reviewers, scoped specialists per area), **escalation triggers** and **hot spots** for the risks the scan surfaced, and the project's severity **calibration**. Read by [pr-review-workflow](pr-review-workflow.md). Same ownership rule as the guardrails: prescriptive, human-owned, never regenerated by refresh — **tell the user it is theirs to edit**.
...
- [ ] `projects/<name>/guardrails.md` and `projects/<name>/pr-review-strategy.md` exist (seeded from their templates) and the user has been told both are theirs to edit.
