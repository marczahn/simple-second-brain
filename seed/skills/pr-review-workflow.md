...
5. **Read in context** — read changed files in full where hunks are non-trivial, plus a neighbouring example of any pattern touched; read the project's `guardrails.md` (authoritative rules), `architecture.md`, `code-conventions.md`, `key-patterns.md`, `architecture-decisions.md`, `testing-conventions.md`, `dependencies.md`. Lean on any `/code-review`-style tooling if available.
